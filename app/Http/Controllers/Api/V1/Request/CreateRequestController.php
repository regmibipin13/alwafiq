    ...
    public function getFirebaseDrivers($request, $type_id)
    {
        ...
        if (!empty($firebase_drivers)) {
            ...
            $nearest_driver_ids = [];
                ...
                $nearest_drivers = Driver::where('active', 1)->where('approve', 1)->where('available', 1)->where('vehicle_type', $type_id)->where('transport_type','taxi')->whereIn('id', $nearest_driver_ids)->whereNotIn('id', $meta_drivers)->limit(10)->get();
